module('Dispatcher');

QUnit.config.reorder = false;

test("is initialize", function () {

    ok(typeof ya.event.$dispatcher !== 'undefined');
});

test("has", function () {

    ok(Array.isArray(ya.event.$dispatcher.getDelegates()), 'array of events');

});

test("may adopt events which will be dispatched", function () {
    var delegates;

    ya.event.$dispatcher.clear();

    ya.event.$dispatcher.add(this, {
        '$list span': {
            click: function () {

            }
        }
    });

    delegates = ya.event.$dispatcher.getDelegates();

    equal(delegates.length, 1);

    ya.event.$dispatcher.add(this, {
        '$list2 span': {
            mouseover: function () {

            }
        }
    });

    equal(delegates.length, 2);

    ya.event.$dispatcher.add(this, {
        '$list span': {
            mouseenter: function () {

            }
        }
    });

    equal(delegates[0].selector.match('^\\$(.*)[\\s]')[1], "list");
    ok(typeof delegates[0].selector !== undefined);

});


test("assign listeners for new view", function () {
    var view;

    ya.event.$dispatcher.add(this, {
        '$test span': {
            click: function (view) {

                view.querySelector('span').innerHTML = '1';

            },
            mouseenter: function () {

            }
        }
    });

    ya.event.$dispatcher.add(this, {
        '$test': {
            click: function () {

            }
        }
    });

    view = ya.View.$create({
        config: {
            id: 'test',
            renderTo: '#test-7',
            tpl: ya.view.Template.$create({
                config: {
                    id: 'tpl-test',
                    tpl: [
                        '<div>',
                        '<ul>',
                        '<li>',
                        '<span>',
                        '</span>',
                        '</li>',
                        '</ul>',
                        '</div>'
                    ]
                }
            })
        }
    });

    view.render();

    ya.event.$dispatcher.apply(view);

    click(view.querySelector('span'));

    equal(view.querySelector('span').innerHTML, '1');

});

test("assign listeners only for appended view", function () {
    var view, view2;

    view = ya.View.$create({
        config: {
            id: 'test2',
            renderTo: '#test-8',
            tpl: ya.view.Template.$create({
                config: {
                    id: 'tpl-test-b',
                    tpl: [
                        '<div>',
                        '<ul>',
                        '<li>',
                        '<span>',
                        '</span>',
                        '</li>',
                        '</ul>',
                        '</div>'
                    ]
                }
            })
        }
    });

    view.render();

    ya.event.$dispatcher.add(this, {
        '$test2 span': {
            click: function (view, el, e) {
                console.log(arguments);
                el.innerHTML = '1';

            }
        }
    });

    view2 = ya.View.$create({
        config: {
            id: 'test2b',
            parent: view,
            renderTo: 'li',
            tpl: ya.view.Template.$create({
                config: {
                    id: 'tpl-test2',
                    tpl: [
                        '<span>',
                        '</span>'
                    ]
                }
            })
        }
    });

    view2.render();

    click(view.querySelectorAll('span')[0]);
    click(view.querySelectorAll('span')[1]);

    console.log(view.querySelectorAll('span'));

    equal(view.querySelectorAll('span')[0].innerHTML, '');
    equal(view.querySelectorAll('span')[1].innerHTML, '1');

});

test("assign listeners for multiple views", function () {

    ya.event.$dispatcher.add(this, {
        '$test3b li': {
            click: function (view, el) {
                el.innerHTML = '1';

            }
        }
    });

    ya.event.$dispatcher.add(this, {
        '$test3 div li': {
            click: function (view, el) {
                el.setAttribute('passed', '1');
            }
        }
    });

    ya.View.$create({
        config: {
            id: 'test3',
            renderTo: '#test-9',
            tpl: ya.view.Template.$create({
                config: {
                    id: 'tpl-test3a',
                    tpl: [
                        '<div>',
                        '</div>'
                    ]
                }
            })
        }
    });

    ya.View.$create({
        config: {
            id: 'test3a',
            parent: ya.view.$manager.getItem('test3'),
            renderTo: 'div',
            tpl: ya.view.Template.$create({
                config: {
                    id: 'tpl-test3b',
                    tpl: [
                        '<div class="top">',
                        '</div>'
                    ]
                }
            })
        }
    });

    ya.View.$create({
        config: {
            id: 'test3b',
            parent: ya.view.$manager.getItem('test3'),
            renderTo: 'div',
            tpl: ya.view.Template.$create({
                config: {
                    id: 'tpl-test3c',
                    tpl: [
                        '<div class="content">',
                        '</div>'
                    ]
                }
            })
        }
    });

    ya.View.$create({
        config: {
            id: 'test3c',
            parent: ya.view.$manager.getItem('test3'),
            renderTo: 'div',
            tpl: ya.view.Template.$create({
                config: {
                    id: 'tpl-test3cd',
                    tpl: [
                        '<div class="bottom">',
                        '</div>'
                    ]
                }
            })
        }
    });

    ya.View.$create({
        config: {
            id: 'test3ba',
            parent: ya.view.$manager.getItem('test3b'),
            renderTo: '.content',
            tpl: ya.view.Template.$create({
                config: {
                    id: 'tpl-test3ba',
                    tpl: [
                        '<ul class="list">',
                        '</ul>'
                    ]
                }
            })
        }
    });


    ya.view.$manager.getItem('test3').render();

    var li1, li2;

    li1 = ya.View.$create({
        config: {
            id: 'test3baa',
            tpl: ya.view.Template.$create({
                config: {
                    id: 'tpl-test3ba-2',
                    tpl: [
                        '<li>',
                        '</li>'
                    ]
                }
            })
        }
    });

    li2 = ya.View.$create({
        config: {
            id: 'test3bab',
            tpl: ya.view.Template.$create({
                config: {
                    id: 'tpl-test3ba-3',
                    tpl: [
                        '<li>',
                        '</li>'
                    ]
                }
            })
        }
    });

    ya.view.$manager.getItem('test3ba').addChild(li1, 'ul');
    ya.view.$manager.getItem('test3ba').addChild(li2, 'ul');

    click(li1.get('el'));
    click(li2.get('el'));

    equal(li1.get('el').innerHTML, '1');
    equal(li1.get('el').getAttribute('passed'), '1');

    equal(li2.get('el').innerHTML, '1');
    equal(li2.get('el').getAttribute('passed'), '1');

//    ya.event.$dispatcher.apply(view2);

});

QUnit.config.reorder = true;